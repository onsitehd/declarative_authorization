module AuthorizationRulesHelper
  ...
  def policy_analysis_hints(marked_up, policy_data)
    ...
    reports_by_line.each do |line, reports|
      text = reports.map { |report| "#{report.type}: #{report.message}" } * " "
  ...
  def privilege_color(privilege, context, role)
    if has_changed(:add_privilege, privilege, context, role)
      ...
      has_changed(:remove_privilege, privilege, context, role) ? '#dd0000' : role_color(role)
    end
  end
  ...
  def describe_step(step, options = {})
    ...
    case step[0]
    when :add_privilege
      dont_assign = prohibit_link(
        step[0, 3],
        "Add privilege <strong>#{h human_privilege_context(step[1], step[2])}</strong> to any role",
        "Don't suggest adding #{h human_privilege_context(step[1], step[2])}.",
        options
      )
    ...
    when :remove_privilege
      dont_remove = prohibit_link(
        step[0, 3],
        "Remove privilege <strong>#{h human_privilege_context(step[1], step[2])}</strong> from any role",
        "Don't suggest removing #{h human_privilege_context(step[1], step[2])}.",
        options
      )
    ...
    when :assign_role_to_user
      dont_assign = prohibit_link(
        ...
        "Don't suggest assigning #{h human_role(step[1].to_sym)}.",
        options
      )
    ...
    when :remove_role_from_user
      dont_remove = prohibit_link(
        ...
        "Don't suggest removing #{h human_role(step[1].to_sym)}.",
        options
      )
