...
class BasicResourcesControllerTest < ActionController::TestCase
  ...
  def test_basic_filter_new_with_params
    ...
    refute @controller.authorized?
    request!(allowed_user, :new, reader, :basic_resource => { :id => "1" }, :clear => [:@basic_resource])
    assert @controller.authorized?
  end
end
...
class NestedResourcesControllerTest < ActionController::TestCase
  def test_nested_filter_index
    ...
    refute @controller.authorized?
    request!(allowed_user, :index, reader, :parent_mock_id => "2", :clear => [:@nested_resource, :@parent_mock])
    refute @controller.authorized?
    request!(allowed_user, :index, reader, :parent_mock_id => "1", :clear => [:@nested_resource, :@parent_mock])
    assert @controller.authorized?
  end
  ...
  def test_nested_filter_show_with_id
    ...
    refute @controller.authorized?
    request!(allowed_user, :show, reader, :id => "1", :parent_mock_id => "1", :clear => [:@nested_resource, :@parent_mock])
    assert @controller.authorized?
  end
  ...
  def test_nested_filter_new_with_params
    ...
    allowed_user = MockUser.new(:allowed_role)
    request!(allowed_user, :new, reader, :parent_mock_id => "2", :nested_resource => { :id => "2" })
    refute @controller.authorized?
    request!(allowed_user, :new, reader, :parent_mock_id => "1", :nested_resource => { :id => "1" }, :clear => [:@nested_resource, :@parent_mock])
    assert @controller.authorized?
  end
end

class ShallowNestedResourcesController < MocksController
  filter_resource_access :nested_in => :parent_mocks, :shallow => true, :additional_member => :additional_member_action, :strong_parameters => false
  define_resource_actions
...
class ShallowNestedResourcesControllerTest < ActionController::TestCase
  def test_nested_filter_index
    ...
    request!(MockUser.new(:another_role), :index, reader, :parent_mock_id => "2")
    refute @controller.authorized?
    request!(allowed_user, :index, reader, :parent_mock_id => "2", :clear => [:@shallow_nested_resource, :@parent_mock])
    refute @controller.authorized?
    request!(allowed_user, :index, reader, :parent_mock_id => "1", :clear => [:@shallow_nested_resource, :@parent_mock])
  ...
  def test_nested_filter_show_with_id
    ...
    request!(allowed_user, :show, reader, :id => "2", :parent_mock_id => "2")
    refute @controller.authorized?
    request!(allowed_user, :show, reader, :id => "1", :clear => [:@shallow_nested_resource, :@parent_mock])
    refute assigns(:parent_mock)
  ...
  def test_nested_filter_additional_member_action_with_id
    ...
    refute @controller.authorized?
    request!(allowed_user, :additional_member_action, reader, :id => "1", :clear => [:@shallow_nested_resource, :@parent_mock])
    refute assigns(:parent_mock)
    assert assigns(:shallow_nested_resource)
    assert @controller.authorized?
  end
...
class CustomMembersCollectionsResourceController < MocksController
  ...
  filter_resource_access :member => [[:other_show, :read]], :collection => { :search => :read }, :new => [:other_new], :strong_parameters => false
...
class CustomMembersCollectionsResourceControllerTest < ActionController::TestCase
  ...
  def test_custom_members_filter_other_new
    ...
    refute @controller.authorized?
    request!(allowed_user, :other_new, reader, :basic_resource => { :id => "1" }, :clear => [:@basic_resource])
    assert @controller.authorized?
  end
end
...
class AdditionalMembersCollectionsResourceController < MocksController
  ...
  filter_resource_access :additional_member => :other_show, :additional_collection => [:search], :additional_new => { :other_new => :new }, :strong_parameters => false
  define_resource_actions
  define_action_methods :other_new, :search, :other_show
end
...
class AdditionalMembersCollectionsResourceControllerTest < ActionController::TestCase
  ...
  def test_additional_members_filter_other_new
    ...
    request!(allowed_user, :other_new, reader, :basic_resource => { :id => "2" })
    refute @controller.authorized?
    request!(allowed_user, :new, reader, :basic_resource => { :id => "2" }, :clear => [:@basic_resource])
    refute @controller.authorized?
