...
class ChangeSupporterTest < Test::Unit::TestCase
  def test_adding_permission
    ...
    approaches = analyzer.find_approaches_for(:users => [user_to_extend_permissions, another_user]) do
      ...
      refute permit?(:read, :context => :permissions, :user => users[1])
    end

    assert approaches.any? do |approach|
      approach.steps.any? do |step|
        step.class == Authorization::DevelopmentSupport::ChangeSupporter::AssignPrivilegeToRoleAction
      end
    end
  end
  ...
  def test_adding_permission_with_new_role_complex
    ...
    approaches = analyzer.find_approaches_for(:users => [another_user, user_to_extend_permissions]) do
      ...
      refute permit?(:read, :context => :permissions, :user => users[0])
    end
  ...
  def test_adding_permission_with_assigning_role_and_adding_permission_with_hierarchy
    ...
    refute approaches.any? do |approach|
  ...
  def test_removing_permission
    ...
    approaches = analyzer.find_approaches_for(:users => [user_to_remove_permissions_from]) do
      refute permit?(:read, :context => :permissions, :user => users.first)
    end
  ...
  def test_removing_permission_privilege_hierarchy
    ...
    assert approaches[0, 2].any? do |approach|
      approach.changes.length == 1 &&
        approach.changes.first.class == Authorization::DevelopmentSupport::ChangeSupporter::RemovePrivilegeFromRoleAction
    end
  end
  ...
  def test_removing_permission_with_constraint
    ...
    assert approaches[0, 2].any? do |approach|
      approach.changes.length == 1 &&
        approach.changes.first.class == Authorization::DevelopmentSupport::ChangeSupporter::RemoveRoleFromUserAction
    end
  end
  ...
  def test_prohibited_actions_role_to_user
    ...
    approaches = analyzer.find_approaches_for(:users => [user_to_extend_permissions], :prohibited_actions => [[:assign_role_to_user, :test_role, user_to_extend_permissions.login]]) do
      assert permit?(:read, :context => :permissions, :user => users.first)
    end
    ...
    refute approaches.any? do |approach|
      approach.steps.any? do |step|
        step.class == Authorization::DevelopmentSupport::ChangeSupporter::AssignRoleToUserAction
      end
    end
  end
  ...
  def test_prohibited_actions_role_to_any_user
    ...
    approaches = analyzer.find_approaches_for(:users => [user_to_extend_permissions], :prohibited_actions => [[:assign_role_to_user, :test_role]]) do
      assert permit?(:read, :context => :permissions, :user => users.first)
    end
  ...
  def test_prohibited_actions_permission_to_role
    ...
    approaches = analyzer.find_approaches_for(:users => [user_to_extend_permissions], :prohibited_actions => [[:add_privilege, :read, :permissions, :test_role_2]]) do
      assert permit?(:read, :context => :permissions, :user => users.first)
    end
  ...
  def test_prohibited_actions_remove_role
    ...
    approaches = analyzer.find_approaches_for(:users => [user_to_remove_permission], :prohibited_actions => [[:remove_role_from_user, :test_role, user_to_remove_permission.login]]) do
      refute permit?(:read, :context => :permissions, :user => users.first)
    end
    ...
    refute approaches.any? do |approach|
      approach.steps.any? do |step|
        step.class == Authorization::DevelopmentSupport::ChangeSupporter::RemoveRoleFromUserAction
      end
    end
  end
  ...
  def test_group_approaches
    ...
    assert approaches.first.similar_to(approaches[1]), "First two approaches should be similar"
