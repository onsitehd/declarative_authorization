...
class MaintenanceTest < Test::Unit::TestCase
  ...
  def test_usages_by_controllers
    ...
    assert Authorization::Maintenance::Usage.usages_by_controller.include?(usage_test_controller)
  end
  ...
  def test_without_access_control
    ...
    refute engine.permit?(:test_2, :context => :permissions, :user => MockUser.new(:test_role))

    Authorization::Maintenance.without_access_control do
      assert engine.permit!(:test_2, :context => :permissions, :user => MockUser.new(:test_role))
    end
