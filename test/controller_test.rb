...
class SpecificMocksController < MocksController
  ...
  filter_access_to :edit_2, :require => :test, :context => :permissions, :attribute_check => true, :model => LoadMockObject
  ...
  define_action_methods(
    ...
    :new,
    :unprotected_action,
    :action_group_action_1,
    :action_group_action_2
  )
end
...
class BasicControllerTest < ActionController::TestCase
  ...
  def test_existing_instance_var_remains_unchanged
    ...
    @controller.send(:instance_variable_set, :"@load_mock_object", mock_object)
    request!(MockUser.new(:test_role), "edit_2", reader)
    assert_equal mock_object, @controller.send(:instance_variable_get, :"@load_mock_object")
    assert @controller.authorized?
  end
...
class LoadMockObjectsController < MocksController
  ...
  filter_access_to :update, :delete, :attribute_check => true, :load_method => proc { MockDataObject.new(:test => 1) }
...
class LoadObjectControllerTest < ActionController::TestCase
  ...
  def test_action_access_with_object_load
    ...
    request!(MockUser.new(:test_role), "show", reader, :id => 1, :clear => [:@load_mock_object])
    assert @controller.authorized?

    request!(MockUser.new(:test_role), "edit", reader, :id => 1, :clear => [:@load_mock_object])
    assert @controller.authorized?
