    ...
    class ChangeSupporter < AbstractAnalyzer
      ...
      class Approach
        ...
        def affected_users(original_engine, original_users, privilege, context)
          (0...@users.length).reject { |i|
            original_engine.permit?(
              privilege,
              :context => context,
        ...
        def state_hash
          @state_hash ||= @engine.auth_rules.inject(0) { |memo, rule|
            memo + rule.privileges.hash + rule.contexts.hash + rule.attributes.hash + rule.role.hash
          }
            .merge(@users.inject(0) { |memo, user| memo + user.role_symbols.hash })
            .merge(@engine.privileges.hash)
            .merge(@engine.privilege_hierarchy.hash)
            .merge(@engine.roles.hash)
            .merge(@engine.role_hierarchy.hash)
        end
        ...
        def similar_to(other)
          other.weight == weight &&
            other.changes.map { |change| change.class.name }.sort == changes.map { |change| change.class.name }.sort
        end
      ...
      class AssignPrivilegeToRoleAction < AbstractAction
        def self.specific_actions(candidate)
          privilege = AnalyzerEngine::Privilege.for_sym(candidate.failed_tests.first.privilege, candidate.engine)
          context = candidate.failed_tests.first.context
          user = candidate.failed_tests.first.user
          ([privilege] + privilege.ancestors).flat_map do |ancestor_privilege|
            user.role_symbols
              .map { |role_sym| AnalyzerEngine::Role.for_sym(role_sym, candidate.engine) }
              .flat_map { |role| [role] + role.ancestors }
              .uniq
              .map { |role|
        ...
        def reverse?(other)
          other.is_a?(RemovePrivilegeFromRoleAction) && other.privilege == @privilege && other.context == @context && other.role == @role
        end
      ...
      class AssignRoleToUserAction < AbstractAction
        ...
        def reverse?(other)
          other.is_a?(RemoveRoleFromUserAction) && other.user.login == user.login && other.role == role
        end
      ...
      class CreateAndAssignRoleToUserAction < AbstractCompoundAction
        def self.specific_actions(candidate)
          ...
          ([privilege] + privilege.ancestors).map do |ancestor_privilege|
      ...
      class AddPrivilegeAndAssignRoleToUserAction < AbstractCompoundAction
        def self.specific_actions(candidate)
          ...
          user = candidate.failed_tests.first.user
          ([privilege] + privilege.ancestors).flat_map do |ancestor_privilege|
            AnalyzerEngine::Role.all(candidate.engine).map do |role|
              new(user, ancestor_privilege.to_sym, context, role.to_sym)
            end
          end
        end
      ...
      class RemovePrivilegeFromRoleAction < AbstractAction
        def self.specific_actions(candidate)
          ...
          ([privilege] + privilege.ancestors).flat_map do |ancestor_privilege|
            user.role_symbols
              ...
              .flat_map { |role| [role] + role.ancestors }
              .uniq
              .map { |role| new(ancestor_privilege.to_sym, context, role.to_sym) }
          end
        end
        ...
        def reverse?(other)
          (other.is_a?(AssignPrivilegeToRoleAction) || other.is_a?(AbstractCompoundAction)) && other.reverse?(self)
        end
      ...
      class RemoveRoleFromUserAction < AbstractAction
        def self.specific_actions(candidate)
          ...
          user.role_symbols.map { |role_sym| AnalyzerEngine::Role.for_sym(role_sym, candidate.engine) }
            .select { |role| roles_for_privilege.include?(role.to_sym) }
            .map { |role| new(user, role.to_sym) }
        end
        ...
        def reverse?(other)
          (other.is_a?(AssignRoleToUserAction) || other.is_a?(AbstractCompoundAction)) && other.reverse?(self)
        end
      ...
      def generate_child_candidates(candidate)
        ...
        abstract_actions.each do |abstract_action|
          abstract_action.specific_actions(candidate).each do |specific_action|
            ...
            if !specific_action.resembles_any?(@prohibited_actions) && !child_candidate.reverse_of_previous?(specific_action) && child_candidate.apply(specific_action)
