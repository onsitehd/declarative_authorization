    ...
    class Analyzer < AbstractAnalyzer
      ...
      class GeneralRulesAnalyzer
        ...
        def mark(type, object)
          @analyzer.reports << Report.new(report_type, source_file(object), source_line(object), message(object))
        end
      ...
      class ProposedPrivilegeHierarchyAnalyzer < GeneralRulesAnalyzer
        ...
        def analyze_privilege(privilege)
          privileges.find do |other_privilege|
            other_privilege != privilege && other_privilege.rules.all? { |rule| rule.privileges.include?(privilege) }
          end
        end
          ...
          if @has_permission
            ...
            permissions_by_context_and_rules.each do |key, rules|
              if rules.length > 1
                ...
                rules.each do |rule|
                  @analyzer.reports << Report.new(:mergeable_rules, "", rule[:line], "Similar rules already in line(s) " + rule_lines.reject { |l| l == rule[:line] } * ", ")
                end
              end
            end
          end
