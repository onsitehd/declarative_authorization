...
module Authorization
  ...
  class Engine
    ...
    def rev_priv_hierarchy
      unless defined?(@rev_priv_hierarchy)
        @rev_priv_hierarchy = {}
    ...
    def permit!(privilege, options = {})
      ...
      privilege = privilege.is_a?(Array) ? privilege.flatten.collect(&:to_sym) : privilege.to_sym
    ...
    def roles_for(user)
      ...
      if defined?(Rails) && Rails.respond_to?(:logger) && !user.respond_to?(:role_symbols)
        Rails.logger.info(
          "The use of user.roles is deprecated. Please add a method " \
  ...
  class AuthorizationRule
    attr_reader :attributes, :contexts, :role, :privileges, :join_operator, :source_file, :source_line

    def initialize(role, privileges = [], contexts = nil, join_operator = :or, options = {})
      @role = role
    ...
    def matches?(roles, privs, context = nil)
      roles = [roles] unless roles.is_a?(Array)
      @contexts.include?(context) && roles.include?(@role) && !(@privileges & privs).empty?
    end

    def validate?(attr_validator, skip_attribute = false)
      skip_attribute || @attributes.empty? || @attributes.send(@join_operator == :and ? :all? : :any?) do |attr|
        begin
  ...
  class AttributeWithPermission < Attribute
    ...
    def obligation(attr_validator, hash_or_attr = nil, path = [])
      ...
      when Symbol
        ...
        obligations = attr_validator.engine.obligations(
          @privilege,
          :context => @context,
      ...
      when Hash
        ...
        obligations = hash_or_attr.each_with_object({}) do |pair, all|
          attr, sub_hash = pair
        ...
        obligations
      when NilClass
        attr_validator.engine.obligations(@privilege, :context => attr_validator.context, :user => attr_validator.user)
      else
        raise AuthorizationError, "Wrong conditions hash format: #{hash_or_attr.inspect}"
      end
    end
