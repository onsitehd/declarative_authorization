...
module Authorization
  ...
  module Reader
    ...
    class AuthorizationRulesReader
      attr_reader :roles, :role_hierarchy, :auth_rules, :role_descriptions, :role_titles, :omnipotent_roles # :nodoc:
      ...
      def has_permission_on(*args, &block)
        ...
        rule = AuthorizationRule.new(@current_role, privs, context, options[:join_by], :source_file => file, :source_line => line)
        @auth_rules << rule
        if block_given?
          @current_rule = rule
          yield
      ...
      def if_permitted_to(privilege, attr_or_hash = nil, options = {})
        ...
        @current_rule.append_attribute AttributeWithPermission.new(privilege, attr_or_hash, options[:context])
      end
