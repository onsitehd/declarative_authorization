...
module Authorization
  module AuthorizationInController
    ...
    def filter_access_filter # :nodoc:
      ...
      unless allowed
        if all_permissions.empty? && matching_permissions.empty?
          logger.warn(
            "Permission denied: No matching filter access " \
        ...
        if respond_to?(:permission_denied, true)
          ...
          send(
            :render,
            :plain => "You are not allowed to access this action.",
            :status => :forbidden
          )
        end
      end
    ...
    def new_controller_object_from_params(context_without_namespace, parent_context_without_namespace, strong_params) # :nodoc:
      ...
      instance_var = :"@#{context_without_namespace.to_s.singularize}"
      instance_variable_set(instance_var, model_or_proxy.new(params[context_without_namespace.to_s.singularize]))
    end
    ...
    module ClassMethods
      ...
      def filter_resource_access(options = {})
        ...
        unless options[:nested_in].blank?
          ...
          before_action only: collections.keys do |controller|
            ...
            if controller.respond_to?(new_for_collection_method, true)
              ...
              controller.send(:new_controller_object_for_collection, options[:context] || controller_name, options[:nested_in], options[:strong_parameters])
            end
          end
        end
        ...
        if options[:strong_parameters]
          ...
          before_action only: :new do |controller|
            ...
            if controller.respond_to?(new_object_method, true)
              ...
              controller.send(:new_blank_controller_object, options[:context] || controller_name, options[:nested_in], options[:strong_parameters], options[:model])
            end
          end
        else
          ...
          before_action only: new_actions.keys do |controller|
            ...
            if controller.respond_to?(new_from_params_method, true)
              ...
              controller.send(:new_controller_object_from_params, options[:context] || controller_name, options[:nested_in], options[:strong_parameters])
            end
          end
        end
        ...
        members.merge(new_actions).merge(collections).each do |action, privilege|
          if action != privilege || (options[:no_attribute_check] && options[:no_attribute_check].include?(action))
            filter_options = {
      ...
      def filter_access_permissions # :nodoc:
        unless filter_access_permissions?
          (ancestors - [self]).reverse_each do |mod|
  ...
  class ControllerPermission # :nodoc:
    ...
    def initialize(actions, privilege, context, strong_params, attribute_check = false, load_object_model = nil, load_object_method = nil, filter_block = nil)
    ...
    def permit!(contr)
      ...
      contr.authorization_engine.permit!(
        privilege,
    ...
    def load_object(contr)
      if @load_object_method && @load_object_method.is_a?(Symbol)
        ...
        load_object_model = @load_object_model || (@context ? @context.to_s.classify.constantize : contr.class.controller_name.classify.constantize)
        ...
        object = contr.instance_variable_defined?(instance_var) && contr.instance_variable_get(instance_var)
        unless object
          begin
            object = @strong_params ? load_object_model.find_or_initialize_by(:id => contr.params[:id]) : load_object_model.find(contr.params[:id])
          rescue => e
            contr.logger.debug(
              "filter_access_to tried to find " \
